...
namespace Equipments.Equipments
{
    public class ZebraCoreScanner
    {
        static ZebraCoreScanner _ZebraCoreScanner = null;
        public static ZebraCoreScanner GetZebraCoreScanner { get { if (_ZebraCoreScanner == null) { _ZebraCoreScanner = new(); _ZebraCoreScanner.Init(); } return _ZebraCoreScanner; } }
        ...
        public eDeviceConnectionStatus CurrentStatus = eDeviceConnectionStatus.InitializationError;
        CCoreScanner CoreScannerObject;
        public ZebraCoreScanner()
        {
        ...
        {
            if (CurrentStatus == eDeviceConnectionStatus.Enabled)
                return eDeviceConnectionStatus.Enabled;
            FileLogger.WriteLogMessage(this, System.Reflection.MethodBase.GetCurrentMethod().Name, "[ZebraCoreScanner] - Start Initialization");
            ...
            try
            {
                ...
                CoreScannerObject.Open(appHandle, // Application handle     
                    ...
                    out status);                  // Command execution success/failure return status 
                                                  //перелік сканерів
                GetInfoScanners(out short countActiveScanners, out string deviceInfoXML, out status);
                if (countActiveScanners <= 0)
                {
                    FileLogger.WriteLogMessage(this, System.Reflection.MethodBase.GetCurrentMethod().Name, "CoreScanner Scanner not found!");
                    CurrentStatus = eDeviceConnectionStatus.InitializationError;
                    return CurrentStatus;
                }

                if (status == (int)CoreScannerLib.Status.Success)
                {
                    FileLogger.WriteLogMessage(this, System.Reflection.MethodBase.GetCurrentMethod().Name, "CoreScanner Open() - Success");
                    ...
                    if (status == (int)CoreScannerLib.Status.Success)
                    {
                        CurrentStatus = eDeviceConnectionStatus.Enabled;
                    }
                    else
                    {
                        CurrentStatus = eDeviceConnectionStatus.InitializationError;
                        FileLogger.WriteLogMessage(this, System.Reflection.MethodBase.GetCurrentMethod().Name, "CoreScanner RegisterForEvents() - Failed. Error Code : " + status);
                    }
            ...
            {
                FileLogger.WriteLogMessage(this, System.Reflection.MethodBase.GetCurrentMethod().Name, "[ZebraCoreScanner] - Initialization Error");
                CurrentStatus = eDeviceConnectionStatus.InitializationError;
                return CurrentStatus;
            }
        }
        private void GetInfoScanners(out short numberOfScanners, out string outXML, out int status)
        {

            numberOfScanners = 0;
            int[] scannerIdList = new int[255];
            CoreScannerObject.GetScanners(out numberOfScanners, scannerIdList, out outXML, out status); // викликати тільки після методу Open !!!!

        }
        private void CoreScannerObject_BarcodeEvent(short eventType, ref string barcodeData)
        {
            ...
            string[] numbers = barcodeDataLabel.Split(' ');
            ...
            string encodedString = System.Text.Encoding.ASCII.GetString(bytes.ToArray());
            //Console.WriteLine($"Barcode : {encodedString}");
            OnBarcodeScannerChange?.Invoke(encodedString);
        }
        ...
        private void OnTimedEvent(object sender, ElapsedEventArgs e)
        {
            ...
            eventIdList[0] = (int)EventType.Barcode;
            string eventIds = String.Join(",", eventIdList);
            string inXml = "<inArgs><scannerID>" + eventIds + "</scannerID></inArgs>";
            int opCode = (int)Opcode.ScaleReadWeight;
            string outXml = "";
            int status = -1;
            CoreScannerObject.ExecCommand(opCode,      // Opcode: Register for events
                                         ref inXml,   // Input XML
                                         out outXml,  // Output XML 
                                         out status); // Command execution success/failure return status
            var document = XDocument.Load(new StringReader(outXml));
            string weight = document.Descendants(XName.Get("weight")).First().Value;
            bool isParse = double.TryParse(weight.Replace('.', ','), out double result);
            //switch (scalStat)
            //{
            //    case 0:
            ...
            //}
            if (!isParse)
                FileLogger.WriteLogMessage(this, System.Reflection.MethodBase.GetCurrentMethod().Name, "[ZebraCoreScanner] - Parse Error");
            string statusWeight = document.Descendants(XName.Get("status")).First().Value;
            if (statusWeight == "6" || statusWeight == "5") // передавати вагу тільки тоді коли вона стабільна або вага занулена
            {
                Action<double> onWeightChanged = OnWeightChanged;
                if (onWeightChanged != null)
                    onWeightChanged(result * 1000);
            }
        }
        public void ForceGoodReadTone()
        {
            int opCode = 2018; //керування звуком
            string outXml;
            int status;
            string beepXML = "<inArgs>" +
                    "<scannerID>1</scannerID>" +
                    "<cmdArgs>" +
                    "<arg-int>" + (int)BeepCode.OneShortLow
                    + "</arg-int>" +
                    "</cmdArgs>" +
                    "</inArgs>";
            CoreScannerObject.ExecCommand(opCode, ref beepXML, out outXml, out status);
        }
        public string GetInfo()
        {
            ...
            GetInfoScanners(out short countActiveScanners, out string deviceInfoXML, out int status);
            var document = XDocument.Load(new StringReader(deviceInfoXML));
            string result = "\n";
            result = result + $"Model number: {document.Descendants(XName.Get("modelnumber")).First().Value} \n";
            result = result + $"Serialnumber: {document.Descendants(XName.Get("serialnumber")).First().Value} \n";
            result += $"Scanner type: {document.Descendants(XName.Get("scanner"))
                                  .FirstOrDefault()?
                                  .Attribute("type")?
                                  .Value ?? "(немає)"} \n";
            result = result + $"GUID: {document.Descendants(XName.Get("GUID")).First().Value} \n";
            result = result + $"VID: {document.Descendants(XName.Get("VID")).First().Value} \n";
            result = result + $"PID: {document.Descendants(XName.Get("PID")).First().Value} \n";
            result = result + $"Date of Manufacture: {document.Descendants(XName.Get("DoM")).First().Value} \n";
            result = result + $"Firmware Version: {document.Descendants(XName.Get("firmware")).First().Value} \n";
            result = result + $"Configuration name: {document.Descendants(XName.Get("configname")).First().Value} \n";
            result = result + $"Status code: {status}";
