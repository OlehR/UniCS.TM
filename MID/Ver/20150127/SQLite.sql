...
[varSqlFindWaresName]
insert into t$1 (id_1) select w.code_wares from wares w where w.name_wares like 
...
[varSqlViewReceipt]
select wr.code_wares code_wares, w.name_wares name_wares ,wr.quantity quantity, ud.abr_unit, wr.sum sum, ' ' type_price
...
[varSqlGetCountWares]
select wr.quantity quantity,wr.sort sort
                     from wares_receipt wr 
                     where wr.id_workplace=@parIdWorkplace and  wr.code_period =@parCodePeriod and  wr.code_receipt=@parCodeReceipt 
                     and wr.code_wares=@parCodeWares and wr.code_unit = @parCodeUnit 
...
[varSqlDeleteWaresReceipt]
 delete from  wares_receipt w
   where id_workplace=@parIdWorkplace and  code_period =@parCodePeriod and  code_receipt=@parCodeReceipt 
                     and code_wares= 
                       case @parCodeWares=0 then code_wares else @parCodeWares end  
    				 and code_unit=
    				 case @parCodeUnit=0 then code_unit else @parCodeUnit end
                     
[varSqlMoveMoney]InputOutputMoney
...
[varSqlLogin]
select u.CODE_USER code_user,p.NAME_FOR_PRINT name_user, u.USER_NAME login,u.PassWord password from users u join privat p on ( u.CODE_USER=p.CODE_PRIVAT) where u.USER_NAME = @parLogin and u.PassWord=@parPassWord
